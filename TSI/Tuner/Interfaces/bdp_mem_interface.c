...
#define BDP_STAT_TOKEN_SET          (1U)
#define BDP_STAT_WAIT_DATA          (2U)
#define BDP_STAT_DATA_SET           (3U)
#define BDP_STAT_ACK_SET            (4U)
#define BDP_STAT_CPLT               (5U)
#define BDP_STAT_DISABLE            (255U)
...
/* Defines ------------------------------------------------------------------*/
/* Device buffer */
static BDP_USED uint8_t tokenBuffer[BDP_TOKEN_PACKET_SIZE];
static BDP_USED uint8_t ackBuffer[BDP_TOKEN_PACKET_SIZE];
static BDP_USED uint8_t dataBuffer[BDP_MAX_DATA_LEN +
                                   BDP_PID_SIZE +
                                   BDP_DATA_CHECKSUM_SIZE];

/* Device variables */
static BDP_USED uint8_t stat;
...
static void MEM_ITF_Init(void);
static void MEM_ITF_DeInit(void);
static void MEM_ITF_Start(void);
static void MEM_ITF_Stop(void);
static void MEM_ITF_Handler(void);
...
static void SetAckPacket(uint8_t ack, uint16_t param);
...
static void MEM_ITF_Reset(void)
{
    /* Init variables */
    stat = BDP_STAT_WAIT_TOKEN;
    memset(tokenBuffer, 0, BDP_TOKEN_PACKET_SIZE);
    ...
    switch(stat) {
        case BDP_STAT_TOKEN_SET:
            ...
            if(transferType == BDP_OUT_TRANSFER && transfer.dataLen > 0U) {
                ...
                SetAckPacket(BDP_ACK_OK, 0U);
                stat = BDP_STAT_WAIT_DATA;
            }
            else {
                /*
                  Enter here if:
        ...
        case BDP_STAT_DATA_SET:
            if(transferType == BDP_OUT_TRANSFER) {
                /* Validate data */
                if(!ValidateDataPacket()) {
                    /* Invalid data */
                    SetAckPacket(BDP_ACK_DATA_ERROR, 0U);
                    stat = BDP_STAT_ACK_SET;
                    break;
                }
                ProcessData();
            };
        ...
        case BDP_STAT_WAIT_DATA:
        case BDP_STAT_WAIT_TOKEN:
        case BDP_STAT_ACK_SET:
        case BDP_STAT_DISABLE:
        default:
            break;
    }
}
...
static void ProcessData(void)
{
    if(transferType == BDP_OUT_TRANSFER) {
        ...
        if(transfer.abort != 0U) {
            ...
            /* Send ACK */
            SetAckPacket(transfer.ack, transfer.ackParams);
            stat = BDP_STAT_ACK_SET;
        }
    }
...
/* Instances ----------------------------------------------------------------*/
const BDP_DevItfTypeDef BDP_MemItf = {
    MEM_ITF_Init,
    MEM_ITF_DeInit,
    MEM_ITF_Reset,
    MEM_ITF_Start,
